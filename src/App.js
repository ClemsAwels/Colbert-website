...
function App() {
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      setTimeout(() => {
        if (window.DruidWebchat_v2) {
          window.DruidWebchat_v2.init({
            botId: "97de0ad6-331a-41fb-840f-08dcff211a6d",
      ...
      }, 100); // 1 seconde de délai pour s'assurer que tout est prêt
    };
    ...
    <div className="flex flex-col min-h-screen p-4 md:p-8 bg-gradient-to-b from-gray-800 to-gray-900 text-gray-200">
      <div className="flex flex-col md:flex-row mt-8 md:mt-12 flex-grow">
        <div className="md:w-1/3 lg:w-1/4 flex flex-col items-center p-4 space-y-8">
          {/* Logo stylisé */}
          <div className="bg-gray-800 rounded-full shadow-lg border-4 border-yellow-200">
            <img src="/logo_colbert.png" alt="Logo" className="h-32 w-32 md:h-40 md:w-40 lg:h-48 lg:w-48 rounded-full" />
          </div>

          {/* Section des Compétences */}
          <div className="text-center bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg w-full mt-4 md:mt-8">
            <h2 className="text-xl md:text-2xl font-semibold text-yellow-200 mb-4">Compétences</h2>
        ...
        <div className="flex flex-col items-center w-full md:w-2/3 lg:w-3/5 max-w-full md:max-w-5xl space-y-4">
          <div className="text-center bg-gray-800 p-4 md:p-6 rounded-t-lg shadow-lg w-full">
            <h1 className="text-2xl md:text-4xl font-extrabold text-yellow-200 mb-4">Bienvenue sur notre chatbot IA spécialisé Colbert !</h1>
            <p className="text-gray-300 mt-2 leading-relaxed text-sm md:text-base">
              Ce chatbot, conçu pour répondre précisément à toutes vos questions en rapport avec le code des impôts, utilise les dernières avancées en intelligence artificielle conversationnelle pour vous offrir une expérience d'assistance interactive et efficace.
            </p>
          </div>
